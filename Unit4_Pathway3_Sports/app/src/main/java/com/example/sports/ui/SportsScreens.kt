    ...
    Scaffold(
        ...
        if (screenWidthDp < 600) {
            if (uiState.isShowingListPage) {
                SportsList(
                    sports = uiState.sportsList, onClick = {
...
@Composable
fun SportsAppBar(
    onBackButtonClick: () -> Unit, isShowingListPage: Boolean, modifier: Modifier = Modifier
) {
    TopAppBar(
        title = {
            Text(
...
private fun SportsListItem(
    sport: Sport, onItemClick: (Sport) -> Unit, modifier: Modifier = Modifier
) {
    Card(
...
private fun SportsList(
    ...
    LazyColumn(
        ...
        items(sports, key = { sport -> sport.id }) { sport ->
            SportsListItem(
                sport = sport, onItemClick = onClick
            )
        }
    }
}
...
fun SportsListItemPreview() {
    SportsTheme {
        SportsListItem(
            sport = LocalSportsDataProvider.defaultSport, onItemClick = {})
    }
}
...
@Composable
fun SportsListAndDetails(viewModel: SportsViewModel, uiState: SportsUiState) {
    Row() {
        SportsList(
            sports = LocalSportsDataProvider.getSportsData(),
            onClick = {
        ...
        SportsDetail(
            selectedSport = uiState.currentSport, onBackPressed = {},
            contentPadding = PaddingValues(0.dp), modifier = Modifier.weight(2f)
        )
    }
}
